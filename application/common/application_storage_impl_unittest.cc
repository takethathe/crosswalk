...
class ApplicationStorageImplTest : public testing::Test {
 public:
  void TestInit() {
    ...
    ASSERT_TRUE(app_storage_impl_->InitDB());
  }
...
TEST_F(ApplicationStorageImplTest, DBInsert) {
  ...
  scoped_ptr<ApplicationData::ApplicationDataMap> applications(
      app_storage_impl_->GetInstalledApplications());
  EXPECT_EQ(applications->size(), 1);
  EXPECT_TRUE((*(applications.get()))[application->ID()]);
}
...
TEST_F(ApplicationStorageImplTest, DBUpgradeToV1) {
  ...
  scoped_ptr<ApplicationData::ApplicationDataMap> applications(
      app_storage_impl_->GetInstalledApplications());
  EXPECT_EQ(applications->size(), 1);
  EXPECT_TRUE((*(applications.get()))["test_id"]);
}
...
TEST_F(ApplicationStorageImplTest, DBUpdate) {
  ...
  scoped_refptr<ApplicationData> saved_application =
      (*(applications.get()))[new_application->ID()];
