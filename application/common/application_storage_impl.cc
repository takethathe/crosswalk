...
bool ApplicationStorageImpl::InitDB() {
...
ApplicationStorageImpl::GetInstalledApplications() {
  if (!db_initialized_) {
    LOG(ERROR) << "The database haven't initilized.";
    return NULL;
  }

  scoped_ptr<ApplicationData::ApplicationDataMap> applications(
      new ApplicationData::ApplicationDataMap);
  ...
  while (smt.Step()) {
    ...
    if (!Insert(application, applications.get())) {
...
bool ApplicationStorageImpl::UpdateApplication(
    const ApplicationData* application, const base::Time& install_time) {
  if (!db_initialized_) {
    LOG(ERROR) << "The database haven't initialized.";
    return false;
  }
...
bool ApplicationStorageImpl::UpdateEvents(
    const std::string &id, const std::vector<std::string>& events) {
  if (!db_initialized_)
    return false;
...
bool ApplicationStorageImpl::SetEventsValue(
  ...
  sql::Transaction transaction(sqlite_db_.get());
  std::string events_list(JoinString(events, kEventSeparator));
